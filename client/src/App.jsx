...
function App() {
  ...
  const [hipaaStatus, setHipaaStatus] = useState(null);
  const [loadingHipaaStatus, setLoadingHipaaStatus] = useState(true);
  ...
  // Check HIPAA compliance status on mount
  useEffect(() => {
    const checkHipaaStatus = async () => {
      try {
        const response = await fetch(`${API_URL}/api/hipaa/status`);
        const data = await response.json();
        setHipaaStatus(data);
        setLoadingHipaaStatus(false);

        // Check if user has already consented in this session
        const consentGiven = sessionStorage.getItem('hipaa_consent_acknowledged');

        if (!consentGiven && data.success) {
          // Show HIPAA notice if consent not yet given
          setShowHipaaNotice(true);
        } else if (consentGiven) {
          setHipaaConsentGiven(true);
        }
      } catch (err) {
        console.error('Failed to check HIPAA status:', err);
        setLoadingHipaaStatus(false);
      }
    };
  ...
  // Handle HIPAA consent acknowledgment
  const acknowledgeHipaaConsent = async () => {
    try {
      const response = await fetch(`${API_URL}/api/hipaa/acknowledge-consent`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          consentAcknowledged: true,
          userIdentifier: 'web-session-' + Date.now()
        })
      });
      ...
      if (data.success) {
        ...
        // Store consent in session storage (not localStorage for additional security)
        sessionStorage.setItem('hipaa_consent_acknowledged', 'true');
        sessionStorage.setItem('hipaa_consent_timestamp', data.timestamp);
      }
    } catch (err) {
      console.error('Failed to acknowledge consent:', err);
      setError('Failed to acknowledge HIPAA consent');
    }
  };
          ...
          <div className="card">
            <div className="tab-header">
              ...
              <button
                ...
                className={`tab ${activeTab === 'emr' ? 'active' : ''}`}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                  <polyline points="14 2 14 8 20 8"/>
...
"72 year old female hospital day 3 with CAP, improving. Overnight no events, no fever. She reports feeling better, cough improving, less SOB. Tolerating PO well. Ambulated with PT yesterday and did well. On exam alert and oriented, lungs with decreased crackles at right base, improved air movement. Plan to transition to oral antibiotics today, continue to monitor, PT to reassess, anticipate discharge tomorrow if continues to improve. Will arrange home health for medication management and follow up with PCP in 1 week."`}
      ...
      {/* HIPAA Compliance Notice Modal */}
      {showHipaaNotice && !loadingHipaaStatus && (
        <div className="modal-overlay hipaa-modal-overlay">
          <div className="modal-content hipaa-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header hipaa-header">
              <h2>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="modal-body hipaa-body">
              <div className="hipaa-warning">
              ...
              <div className="hipaa-content">
                <p className="hipaa-intro">
                  This application processes Protected Health Information (PHI) and clinical data.
                ...
                <div className="hipaa-section">
                  ...
                  <ul>
                    <li><strong>Automatic PHI Redaction:</strong> The system automatically detects and redacts 18 HIPAA identifiers including names, addresses, dates, phone numbers, SSN, MRN, and other identifying information before sending data to AI services.</li>
                    <li><strong>Image Metadata:</strong> Medical images may contain embedded PHI in DICOM/EXIF metadata. Ensure images are properly de-identified before upload.</li>
                    <li><strong>Session-Based:</strong> No PHI is permanently stored. All data is processed in-memory during your session only.</li>
                  </ul>
                </div>

                {hipaaStatus && !hipaaStatus.baaAcknowledged && (
                  <div className="hipaa-section hipaa-critical">
                    <h4>⚠️ Business Associate Agreement Required</h4>
                    <ul>
                      <li><strong>BAA with Anthropic:</strong> This application uses Anthropic's Claude API. Under HIPAA, a Business Associate Agreement (BAA) with Anthropic is <strong>required</strong> before processing any PHI.</li>
                      <li><strong>Enterprise Requirement:</strong> Anthropic BAAs are available for enterprise customers. Contact Anthropic sales to establish a BAA.</li>
                      <li><strong>Compliance Risk:</strong> Using this application with PHI without a BAA may violate HIPAA regulations.</li>
                    </ul>
                  </div>
                )}
                ...
                <div className="hipaa-section">
                  ...
                  <ul>
                    <li>Do not include unnecessary PHI - use minimum necessary principle</li>
                    <li>Verify PHI redaction is functioning (check console logs for "[PHI Protection]" messages)</li>
                    <li>Manually review and de-identify data when possible</li>
                    <li>Do not upload images with patient faces or visible identifying information</li>
                    <li>Ensure your organization has appropriate BAA(s) in place</li>
                    <li>Follow your organization's HIPAA policies and procedures</li>
                  </ul>
                </div>

                {hipaaStatus && hipaaStatus.warnings?.length > 0 && (
                  <div className="hipaa-section hipaa-warnings">
                    <h4>⚡ System Status</h4>
                    <ul>
                      {hipaaStatus.warnings.map((warning, i) => (
                        <li key={i}>{warning}</li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="hipaa-section">
                  ...
                  <ul>
                    <li><a href="https://www.hhs.gov/hipaa" target="_blank" rel="noopener noreferrer">HHS HIPAA Information</a></li>
                    <li><a href="https://www.anthropic.com/legal/business-associate-agreement" target="_blank" rel="noopener noreferrer">Anthropic BAA Information</a></li>
                  </ul>
                </div>

                <div className="hipaa-disclaimer">
                  <strong>Clinical Disclaimer:</strong> This is a decision support tool only. All AI-generated content requires professional clinical verification. Not a substitute for professional medical judgment or consultation.
                </div>
              </div>
            </div>

            <div className="modal-footer hipaa-footer">
              <label className="hipaa-checkbox-label">
                <input
                  type="checkbox"
                  checked={hipaaConsentGiven}
                  onChange={(e) => setHipaaConsentGiven(e.target.checked)}
                  className="hipaa-checkbox"
                />
                <span>
                  I acknowledge that I have read and understand the HIPAA compliance requirements,
                  PHI protection measures, and my responsibilities when using this application.
                  {hipaaStatus && !hipaaStatus.baaAcknowledged && (
                    <strong> I confirm that my organization has a Business Associate Agreement with Anthropic.</strong>
                  )}
                </span>
              </label>

              <button
                onClick={acknowledgeHipaaConsent}
                disabled={!hipaaConsentGiven}
                className="btn-primary hipaa-btn"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
